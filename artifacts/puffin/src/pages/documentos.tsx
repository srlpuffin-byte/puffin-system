          ...
          <div className="rounded-md border overflow-hidden">
            ...
            <div className="md:hidden divide-y">
              {isLoading ? (
                ...
                documentos?.map((doc) => (
                  <div key={doc.id} className="p-4 flex flex-col gap-3 hover:bg-slate-50 transition-colors">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex flex-col">
                        <span className="font-semibold text-primary capitalize leading-tight">{doc.tipo}</span>
                        <span className="text-xs text-muted-foreground mt-0.5">{doc.entidad_nombre || "-"}</span>
                      </div>
                      <Badge
                        variant={doc.estado === "vigente" ? "default" : doc.estado === "vencido" ? "destructive" : "outline"}
                        className={
                          doc.estado === "vigente" ? "bg-green-600" :
                          doc.estado === "proximo_vencimiento" ? "text-yellow-600 border-yellow-600" : ""
                        }
                      >
                        {doc.estado?.toUpperCase().replace("_", " ")}
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm bg-slate-50 p-2 rounded border mt-1">
                      <div className="flex flex-col">
                        <span className="text-[10px] uppercase text-muted-foreground font-semibold">Vencimiento</span>
                        <span className="font-medium">
                          {doc.fecha_vencimiento ? format(new Date(doc.fecha_vencimiento), "dd/MM/yyyy") : "-"}
                        </span>
                      </div>
                      <div className="flex flex-col items-end">
                        <span className="text-[10px] uppercase text-muted-foreground font-semibold">Tiempo Restante</span>
                        <span className={`font-medium ${
                          (doc.dias_restantes !== undefined && doc.dias_restantes <= 0) ? "text-red-600 font-bold" :
                          (doc.dias_restantes !== undefined && doc.dias_restantes <= 30) ? "text-yellow-600 font-bold" : ""
                        }`}>
                          {doc.dias_restantes !== undefined
                            ? (doc.dias_restantes <= 0 ? `Hace ${Math.abs(doc.dias_restantes)} días` : `${doc.dias_restantes} días`)
                            : "-"}
                        </span>
                      </div>
                    </div>
