...
import { Bell, MessageSquare, AlertTriangle, CheckCircle, Sparkles, Send, Smartphone } from "lucide-react";
...
export function NotificationBell() {
  ...
  const { unreadWhatsAppCount, recentNotifications } = useGlobalNotifications();
  const {
    isSupported,
    permission,
    isSubscribed,
      ...
      >
        ...
        <div className="p-3.5 bg-muted/40 border-b border-border flex items-center justify-between">
          ...
          {unreadWhatsAppCount > 0 && (
            <Badge variant="secondary" className="text-xs bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300">
              {unreadWhatsAppCount} sin leer en WhatsApp
            </Badge>
          )}
        </div>
        ...
        <div className="max-h-[300px] overflow-y-auto divide-y divide-border/60">
          {recentNotifications && recentNotifications.length > 0 ? (
            recentNotifications.slice(0, 10).map((item) => {
            ...
            <div className="py-8 text-center px-4">
              ...
              <p className="text-[11px] text-muted-foreground/70 mt-0.5">
                Los mensajes de WhatsApp y avisos del sistema aparecerán aquí en vivo.
              </p>
            </div>
          )}
        </div>

        {/* Pie de página con acceso rápido */}
