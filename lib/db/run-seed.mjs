...
async function seed() {
  ...
  // 2. Crear usuario admin (desbloquear si estaba bloqueado)
  const pinHash = hashPin('1234');
  const result = await client.query(`
    INSERT INTO usuarios (empresa_id, nombre, apellido, usuario, pin_hash, rol, activo, bloqueado, intentos_fallidos)
    VALUES ($1, 'Administrador', 'Sistema', 'admin', $2, 'admin', true, false, 0)
    ON CONFLICT (usuario) DO UPDATE SET pin_hash = $2, activo = true, bloqueado = false, intentos_fallidos = 0
    RETURNING id, usuario, rol, bloqueado
  `, [empresaId, pinHash]);
